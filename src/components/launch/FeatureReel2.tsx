...
function FeatureCard({
    ...
    return (
        <div
            ...
            className={`relative rounded-2xl border overflow-hidden cursor-default transition-all duration-500 ${isDesktop ? "w-[390px] lg:w-[420px] h-[340px] lg:h-[360px] flex-shrink-0" : "w-full"
                } ${alive
            ...
            <div className={`relative z-10 h-full flex flex-col justify-between ${isDesktop ? "p-8" : "p-6"}`}>
                {feature.image ? (
                    <>
                        ...
                        {/* Image on top-left */}
                        <div className={`flex-1 flex items-start justify-start ${feature.title === "Lip Sync" ? "-mt-4 -ml-4" : feature.title === "Voice Cloning" ? "-mt-8 -ml-14" : feature.title === "Auto Subtitles" || feature.title === "YouTube Integration" || feature.title === "Multi-Speaker" ? "-mt-4 -ml-4" : "-mt-7 -ml-8"}`}>
                            <img
                                src={feature.image}
                                alt={feature.title}
...
export default function FeatureReel2() {
    return (
        <>
            <DesktopReel />
            <MobileStack />
